...
#     except ValueError:
#         return str(int(row['GSS']))  # Or any other appropriate handling for non-numeric values
...
def process_file(filename):
    ...
    with pd.ExcelWriter(output_filepath) as writer:
        ...
        Four.to_excel(writer, sheet_name='Four')
        Five.to_excel(writer, sheet_name='Five')
        Six.to_excel(writer, sheet_name='Six')
        Seven.to_excel(writer, sheet_name= 'Seven')
        Eight.to_excel(writer, sheet_name='Eight', index=False)
        Nine.to_excel(writer, sheet_name='Nine', index=False)
